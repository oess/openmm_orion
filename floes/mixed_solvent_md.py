...
from MDOrion.MDEngines.cubes import (
    ParallelMDMinimizeCube,
    ParallelMDNvtCube,
    ParallelMDNptCube,
)

from MDOrion.System.cubes import ParallelMixedSolventSolvationCube, MDComponentCube

from MDOrion.ForceField.cubes import ParallelForceFieldCube

from MDOrion.System.cubes import (
    CollectionSetting,
)
from MDOrion.TrjAnalysis.cubes_occupancy import OccupancyCalculator

job = WorkFloe("Mixed Solvent Pocket Sampling", title="Mixed Solvent Pocket Sampling")

job.description = """Sample Proteins with mixed solvents to find cryptic pockets"""
...
solvate = ParallelMixedSolventSolvationCube("Solvation", title="Solvation")
...
ff.promote_parameter(
    "protein_forcefield", promoted_name="protein_ff", default="Amber14SB"
)
ff.promote_parameter(
    "ligand_forcefield", promoted_name="ligand_ff", default="OpenFF_1.2.1"
)
...
prod.modify_parameter(
    prod.restraint_to_reference,
    promoted_name="restraint_to_reference",
    default=False,
    description="Restrain protein to reference",
)
...
# Minimization
minimize = ParallelMDMinimizeCube("minimize", title="Minimization")
minimize.modify_parameter(
    minimize.restraints, promoted=False, default="noh (ligand or protein)"
)
minimize.modify_parameter(minimize.restraintWt, promoted=False, default=5.0)
minimize.modify_parameter(minimize.steps, promoted=False, default=0)
minimize.set_parameters(center=True)
minimize.set_parameters(save_md_stage=True)
minimize.set_parameters(hmr=False)
minimize.promote_parameter("md_engine", promoted_name="md_engine")
...
warmup.modify_parameter(
    warmup.restraints, promoted=False, default="noh (ligand or protein)"
)
...
equil1.modify_parameter(
    equil1.restraints, promoted=False, default="noh (ligand or protein)"
)
...
equil2.modify_parameter(
    equil2.restraints, promoted=False, default="noh (ligand or protein)"
)
...
equil3.modify_parameter(
    equil3.restraints, promoted=False, default="noh (ligand or protein)"
)
...
equil4.modify_parameter(
    equil4.restraints, promoted=False, default="ca_protein or (noh ligand)"
)
...
md_group = ParallelCubeGroup(
    cubes=[minimize, warmup, equil1, equil2, equil3, equil4, prod]
)
...
occupancy = OccupancyCalculator("occupancy")
...
# Runs both individually and solvates without ligand
mdcomp.success.connect(solvate.intake)
...
coll_close.success.connect(occupancy.intake)
occupancy.success.connect(ofs.intake)
